...
The `Dog ownership` dataset comes from the Brisbane City Council (BCC) [open data portal](https://www.data.brisbane.qld.gov.au/data/dataset/current-animal-related-permits/resource/c2823090-8a44-4c8b-9ed4-e988491b25f4). We will use the `CARS - BIS - Open Data - Animal Permits Breed - 1 Jul 2019.csv` dataset that is closest in time to the `SEIFA` dataset. BBC's data portal does not provide an earlier version of their data. Unfortunately, our attempts to contact the BCC and obtain an earlier version were not successful. In case the earlier dataset becomes available we will use that instead.
...
In case of inconsistencies, mismatches or name conflicts we will use the `Locality boundaries - Queensland` data from the from Queensland Spatial Catalogue - [QSpatial](http://qldspatial.information.qld.gov.au/catalogue/custom/detail.page?fid={8F24D271-EE3B-491C-915C-E7DD617F95DC).
