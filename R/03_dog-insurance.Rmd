...
pca_results <- wide_insurance_p %>%
  ...
  mutate(pca_raw = predict(pca)$PC1, 
...
```{r}
cluster_results %>% 
  select(cluster, alaskan_malamute:welsh_corgi_cardigan) %>% 
  pivot_longer(alaskan_malamute:welsh_corgi_cardigan) %>% 
