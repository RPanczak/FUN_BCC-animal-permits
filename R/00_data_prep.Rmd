...
Full (?) list of Brisbane suburbs. Top 5 alphabetically:
...
```{r}
dog_ownership <- read_csv("data/permits/raw/cars-bis-open-data-animal-permits-3-jan-2019.zip")
```

Raw dataset consists of `r comma(nrow(dog_ownership))`
...
*Stones Corner* values were assigned to *Greenslopes* SSC (see above).

```{r}
dog_ownership %<>% 
  clean_names() %>%
  remove_empty(c("rows", "cols")) %>% 
  select(-permit_group, -permit_status) %>% # all the same here
...
```{r}
saveRDS(dog_ownership, "data/permits/clean/dog_ownership.Rds") 
```

```{r}
dog_ownership <- readRDS("data/permits/clean/dog_ownership.Rds") 
```
...
Merge data of `dog_cost` and `dog_ownership`, and `dog_insurance` and `dog_ownership`, respectively. Aim: dog breeds should have same name in all data sets.

```{r}

# rename dog_ownership dog names to match dog_cost names
table(dog_ownership$dog_breed)

dog_ownership <- dog_ownership %>%
  mutate(
    dog_breed = case_when(
      dog_breed == "Akita (Japanese)" ~ "Akita",
      dog_breed == "Dogue de Bordeaux" ~ "French Mastiff",
      dog_breed == "British Bulldog" ~ "English Bulldog",
      dog_breed == "Bulldog" ~ "English Bulldog",
      dog_breed == "German Shepherd Dog (Long Stock Coat)" ~ "German Shepherd",
      dog_breed == "German Shepherd Dog" ~ "German Shepherd",
      dog_breed == "Chinese Crested Dog" ~ "Hairless Chinese Crested",
      dog_breed == "St Bernard" ~ "Saint Bernard",
      TRUE ~ as.character(dog_breed)))

# binary indicator "expensive" and "non-expensive" dog breeds according to dog_cost
dog_cost$expensive <- "yes"

dog_ownership_cost <- left_join(dog_ownership, dog_cost, by = "dog_breed")

dog_ownership_cost <- dog_ownership_cost %>% 
  mutate(
    expensive = case_when(
      is.na(expensive) ~ 0,
      expensive == "yes" ~ 1
...
```{r}
saveRDS(dog_ownership_cost, "data/permits/clean/dog_ownership_cost.Rds") 
```

```{r}
dog_ownership_cost <- readRDS("data/permits/clean/dog_ownership_cost.Rds") 
```

```{r}
# graph expensive dogs

dog_ownership_cost %>% 
  mutate(dog_breed = fct_lump(dog_breed, n = 50)) %>%
  ggplot(aes(x = dog_breed, fill = expensive)) + 
  geom_bar() +
  coord_flip()
...
```{r}
# rename dog_insurance names to match dog_cost names.

# rename dog_ownership names to match dog_insurance dog names. 
