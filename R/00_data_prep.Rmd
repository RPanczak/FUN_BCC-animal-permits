...
library(pacman) 
p_load(tidyverse, readxl, janitor, lubridate, sjmisc,
       sf, tmap, rmapshaper)
...
```{r include=FALSE}
## Source 1
dog_cost_1 <- read_xlsx("data/costs/raw/dog_cost.xlsx") %>% 
  clean_names() %>%
  remove_empty(c("rows", "cols")) %>% 
  # fill(web_source)
  select(-web_source) %>% 
...
## Source 2
dog_cost_2 <- read_xlsx("data/costs/raw/dog_cost.xlsx") %>% 
  clean_names() %>%
  remove_empty(c("rows", "cols")) %>% 
  select(-web_source) %>% 
...
## Combined
dog_cost <- 
  select(dog_cost_1, dog_breed) %>% 
  bind_rows(select(dog_cost_2, dog_breed)) %>% 
  distinct() %>% 
  arrange(dog_breed)

rm(dog_cost_1, dog_cost_2)
