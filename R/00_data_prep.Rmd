...
saveRDS(LB, "data/geo/Locality_Boundaries.Rds")
```
...
```{r}
seifa <- read_xls("data/SEIFA/2033055001 - ssc indexes.xls", 
  ...
  dplyr::rename(SSC_CODE16 = x1,
    ...
    mutate(SSC_CODE16 = as.integer(SSC_CODE16),
         STE_CODE16 = as.integer(STE_CODE16)) %>% 
  mutate(caution = ifelse(is.na(caution), "False", "True")) %>% 
  mutate(caution = as.logical(caution)) %>% 
  left_join(SSC)
