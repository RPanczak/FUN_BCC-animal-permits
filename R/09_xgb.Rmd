...
```{r}
p_xgb_tu <- predict(m_xgb_tu, data_x)
```
...
```{r}
ModelMetrics::mae(data_y, p_xgb_tu)
```

RMSE:  

```{r}
ModelMetrics::rmse(data_y, p_xgb_tu)
```
...
ggplot() +
  geom_density(data = as.data.frame(p_xgb_tu), aes(x = p_xgb_tu), col = "firebrick") +
  geom_density(data = as.data.frame(data_y), aes(x = data_y), col = "forestgreen") 
```
